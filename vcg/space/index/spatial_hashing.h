...
public:
    ...
    inline bool Empty() const { return hash_table.empty(); }
...
public:
    ...
    int CountInSphere(const Point3<ScalarType> &p,
                      ...
                      std::vector<HashIterator> &inSphVec)
    {
        ...
        for (int i = bb.min.X(); i <= bb.max.X(); i++)
            for (int j = bb.min.Y(); j <= bb.max.Y(); j++)
                for (int k = bb.min.Z(); k <= bb.max.Z(); k++) {
                    std::pair<HashIterator, HashIterator> CellRange = hash_table.equal_range(
                        Point3i(i, j, k));
    ...
    size_t RemoveInSphere(const Point3<ScalarType> &p, const ScalarType radius)
    {
        ...
        for (typename std::vector<HashIterator>::iterator vi = inSphVec.begin();
             vi != inSphVec.end();
             ++vi)
            hash_table.erase(*vi);
    ...
    int RemoveInSphereNormal(const Point3<ScalarType> &p,
                             ...
                             const ScalarType radius)
    {
        ...
        for (typename std::vector<HashIterator>::iterator vi = toDel.begin(); vi != toDel.end();
             ++vi)
            hash_table.erase(*vi);
    ...
    {
        return (vcg::GridClosest<SpatialHashType, OBJPOINTDISTFUNCTOR, OBJMARKER>(
            *this, _getPointDistance, _marker, _p, _maxDist, _minDist, _closestPt));
    }

    template<class OBJPOINTDISTFUNCTOR,
             class OBJMARKER,
             class OBJPTRCONTAINER,
             class DISTCONTAINER,
             class POINTCONTAINER>
    unsigned int GetKClosest(OBJPOINTDISTFUNCTOR &_getPointDistance,
                             OBJMARKER &_marker,
    ...
    {
        return (vcg::GridGetInBox<SpatialHashType, OBJMARKER, OBJPTRCONTAINER>(*this,
                                                                               _marker,
                                                                               _bbox,
                                                                               _objectPtrs));
    }
    ...
    ObjPtr DoRay(OBJRAYISECTFUNCTOR &_rayIntersector,
                 OBJMARKER &_marker,
                 const Ray3<ScalarType> &_ray,
                 const ScalarType &_maxDist,
                 ScalarType &_t)
    {
        return (vcg::GridDoRay<SpatialHashType, OBJRAYISECTFUNCTOR, OBJMARKER>(*this,
                                                                               _rayIntersector,
                                                                               _marker,
                                                                               _ray,
                                                                               _maxDist,
